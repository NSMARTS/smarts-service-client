...
export class DrawStoreService {
  ...
  setDrawEvent(pageNum: number, drawingEvent: any) {
    ...
    if (itemIndex < 0) {
      this.drawVar.mutate(array => array.push({ pageNum: pageNum, drawingEvent: [drawingEvent] }))
    }
    // 해당 page의 data가 있는 경우, 기존 data에 event 추가
      ...
      this.drawVar.mutate(array => array[itemIndex] = { ...array[itemIndex], drawingEvent: updatedDrawingEvents });
    };
  }
  ...
  clearDrawingEvents(pageNum: number) {
    this.drawVar.mutate(array => array.filter(item => item.pageNum !== pageNum))
  }
