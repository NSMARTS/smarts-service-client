...
import { Observable, catchError, lastValueFrom, shareReplay, tap, throwError } from 'rxjs';
...
export class AuthService {
    ...
    constructor(
        ...
        effect(() => { console.log(this.accessToken()) })
        effect(() => { console.log(this.userInfoStore()) })
    }

    signIn(signInForm: SignIn) {
        return this.http.post<AccessToken>(this.baseUrl + '/auth/signIn', signInForm).pipe(
            tap(data =>
                this.accessToken.set(data) // access token 등록
            ),
            tap(data => {
                // jwt를 디코딩 한 후 상태관리
    ...
    signUp(signUpForm: SignUp) {
        return this.http.post<AccessToken>(this.baseUrl + '/auth/signUp', signUpForm)
    }
